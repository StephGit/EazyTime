...
open class GeoFenceBaseFragment : androidx.fragment.app.Fragment() {
    ...
    private val addGeoFenceRequest = 1
    ...
    protected fun addGeoFence() {
        ...
        if (permissionFineLocationGranted) {
            startActivityForResult(GeoFenceDetailActivity.newIntent(context!!), addGeoFenceRequest)
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == addGeoFenceRequest && resultCode == Activity.RESULT_OK) {
